...
class Sales extends CI_Controller
{
    ...
    public function addSales()
    {
        ...
        try {
            ...
            $sales = array(
                ...
                'exchange_note' => $data->sales->exchangeNote,
            ...
            if (isset($data->exchange)) {
                foreach ($data->exchange as $product) {
                    $exchange = array(
                        'sale_id' =>  $salesId,
                    ...
                    $this->db->query("
                        update tbl_currentinventory 
                        set exchange_quantity = exchange_quantity + ? 
                        where product_id = ?
                        and branch_id = ?
    ...
    public function updateSales()
    {
        ...
        try {
            ...
            $currentExchange = $this->db->query("select * from tbl_exchange  where sale_id = ?", $salesId)->result();
            $this->db->query("delete from tbl_exchange where sale_id = ?", $salesId);

            foreach ($currentExchange as $product) {
                $this->db->query("
                    update tbl_currentinventory 
                    set exchange_quantity = exchange_quantity - ? 
                    where product_id = ?
...
<?php
    ...
    public function  deleteSales()
    {
        ...
        try {
            ...
            /* exchange details */
            $exchangeDetails = $this->db->query("select * from tbl_exchange where sale_id = ?", $saleId)->result();

            foreach ($exchangeDetails as $product) {
                $this->db->query("
                    update tbl_currentinventory 
                    set exchange_quantity = exchange_quantity - ? 
                    where product_id = ?
            ...
            // delete exchange
            $this->db->set('status', 'd')->where('sale_id', $saleId)->update('tbl_exchange');
