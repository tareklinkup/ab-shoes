...
class Purchase extends CI_Controller
{
    ...
    public function getPurchases()
    {
        ...
        if (isset($data->memoId) && $data->memoId != '') {
            $clauses .= " and pm.memo_no = '$data->memoId'";
        }
    ...
    public function order()
    {
        ...
        $productModel = $this->mt->generateProductModel();
    ...
    public function addPurchase()
    {
        ...
        try {
            ...
            $purchase = array(
                ...
                'PurchaseMaster_labour_cost'    => $data->purchase->labour_cost,
            ...
            foreach ($data->cartProducts as $product) {
                // insert product $this->mt->generateProductCode();
                $checkModel = $this->db->query("select * from tbl_product where model= ?", $product->model);

                if ($checkModel->num_rows() > 0) {

                    $productId = $checkModel->row()->Product_SlNo;
                    ...
                    $proData = array(
                        ...
                        'Product_WholesaleRate' => $product->salesRate,
                        'status'                => 'a',
                        'UpdateBy'              => $this->session->userdata('FullName'),
                        'UpdateTime'            => date('Y-m-d H:i:s'),
                    ...
                    $this->db->where('Product_SlNo', $productId)->update('tbl_product', $proData);
                    ...
                    $this->db->insert('tbl_product', $proData);
                ...
                if ($inventoryCount == 0) {
                    ...
                    $this->db->query("
                        ...
                        where product_id = ? 
                        and branch_id = ?
                    ", [$product->quantity, $productId, $this->session->userdata('BRANCHid')]);
                }
            }
    ...
    public function updatePurchase()
    {
        ...
        try {
            ...
            foreach ($data->cartProducts as $product) {
                ...
                $existCheck = $this->db->query("SELECT * FROM `tbl_product` WHERE `model` = ? and status = 'a'", $product->model)->row();
                ...
                if ($existCheck) {
                    ...
                    $this->db->where('Product_SlNo', $existCheck->Product_SlNo)->update('tbl_product', $proData);
                    $productId =  $existCheck->Product_SlNo;
                } else {
                    $proData = array(
                        'supplier_id'           => $supplierId,
                        'Product_Code'          => $this->mt->generateProductCode(),
                ...
                $previousStock = $this->mt->productStock($productId);
    ...
    public function getDamages()
    {
        ...
        $damages = $this->db->query("
            ...
            join tbl_brand b on b.brand_SiNo = p.brand
            join tbl_color c on c.color_SiNo = p.color
            join tbl_size sz on sz.id = p.size
    ...
    public function damageReturn()
    {
        $access = $this->mt->userAccess();
        if (!$access) {
            redirect(base_url());
        }
        $data['title'] = "Damage Return";

        $data['content'] = $this->load->view('Administrator/purchase/damage_return', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function damageForReturns()
    {
        $data = json_decode($this->input->raw_input_stream);
        ...
        $damages = $this->db->query("
            select 
                dd.*,
                dd.damage_rate as return_rate,
                p.Product_Code,
                p.Product_Name,
                (
                    select 
                        ifnull(sum(rd.quantity), 0)
                    from tbl_damage_returndetails rd
                    where rd.product_id = p.Product_SlNo
                    and rd.status = 'a'
                ) as returned_quantity,
                (
                    select 
                        ifnull(sum(rd.amount), 0)
                    from tbl_damage_returndetails rd
                    where rd.product_id = p.Product_SlNo
                    and rd.status = 'a'
                ) as returned_amount
            ...
            and dd.Damage_SlNo = ?
        ", [$this->session->userdata('BRANCHid'), $data->damageId])->result();

        echo json_encode($damages);
    }

    public function addDamageReturn()
    {
        $res = new stdClass;
        $this->db->trans_begin();
        try {
            ...
            $damageReturn = array(
                'date' => $data->damageReturn->date,
                'invoice' => $data->damageReturn->invoice,
                'total' => $data->damageReturn->total,
                'note' => $data->damageReturn->note,
                'status' => 'a',
                'added_by' => $this->session->userdata('FullName'),
                'addded_time' => date('Y-m-d H:i:s'),
                'branch_id' => $this->session->userdata('BRANCHid')
            );

            $this->db->insert('tbl_damage_return', $damageReturn);
            ...
            foreach ($data->cart as $product) {
                $detail = array(
                    ...
                    'amount' => $product->return_amount,
                    'status' => 'a',
                    'branch_id' => $this->session->userdata('BRANCHid')
                );

                $this->db->insert('tbl_damage_returndetails', $detail);

                $this->db->query("
                    update tbl_currentinventory 
                    set damage_return = damage_return + ? 
                    where product_id = ?
                    and branch_id = ?
                ", [$product->return_quantity, $product->Product_SlNo, $this->session->userdata('BRANCHid')]);
            }
