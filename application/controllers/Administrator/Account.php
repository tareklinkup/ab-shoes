...
class Account extends CI_Controller
{
    ...
    function getOtherIncomeExpense()
    {
        ...
        $result = $this->db->query("
            ...
            (
                select ifnull(sum(pm.PurchaseMaster_labour_cost), 0) 
                from tbl_purchasemaster pm
                where pm.PurchaseMaster_BranchID = '" . $this->session->userdata('BRANCHid') . "'
                and pm.status = 'a'
                $purchaseClause
            ) as purchase_labour_cost,
    ...
    public function cashDrawer()
    {
        ...
        $data['content'] = $this->load->view('Administrator/account/cash_drawer', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }

    public function cashDrawerRecord()
    {
        $access = $this->mt->userAccess();
        if (!$access) {
            redirect(base_url());
        ...
        $data['title'] = "Cash Drawer Record";
        $data['content'] = $this->load->view('Administrator/account/cashdrawer_list', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    ...
    public function addCashDrawer()
    {
        ...
        try {
            ...
            $this->db->insert('tbl_drawer', $drawer);
    ...
    public function updateCashDrawer()
    {
        ...
        try {
            ...
            $this->db->where('id', $data->id)->update('tbl_drawer', $drawer);
    ...
    public function deleteCashDrawer()
    {
        ...
        try {
            ...
            $this->db->where('id', $data->id)->set('status', 'd')->update('tbl_drawer');
