...
class Employee extends CI_Controller
{
    ...
    public function employee_insert()
    {
        ...
        $data['bank_account'] = $this->input->post('account', true);
    ...
    public function getPayments()
    {
        ...
        $payments = $this->db->query("SELECT ep.*, e.Employee_Name,e.Employee_ID,m.month_name
        FROM tbl_employee_payment ep
        left JOIN tbl_employee e on e.Employee_SlNo = ep.Employee_SlNo
        left join tbl_month m on m.month_id = ep.month_id
        WHERE ep.status = 'a'
        $clauses
        and ep.Branch_id = ?", $this->session->userdata("BRANCHid"))->result();
    ...
    public function getSalaryDetails()
    {
        ...
        if (isset($data->employee_id) && $data->employee_id != '') {
            $clauses .= " and ep.Employee_SlNo = '$data->employee_id'";
        }
    ...
    public function generateEmployeeSalary()
    {
        ...
        dp.Department_Name,
         ...
         join tbl_employee e on e.Employee_SlNo = es.Employee_ID
            join tbl_department dp on dp.Department_SlNo = e.Department_ID
            join tbl_designation ds on ds.Designation_SlNo = e.Designation_ID
         WHERE Month_Id = ? and Branch_Id = ?", [$data->monthId, $this->session->userdata('BRANCHid')])->result();
