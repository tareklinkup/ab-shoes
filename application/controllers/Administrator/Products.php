...
class Products extends CI_Controller
{
    ...
    public function check_product()
    {
        ...
        $query = $this->db->query("SELECT p.* FROM tbl_product p WHERE p.status = 'a' AND p.Product_Name = ? AND p.Product_branchid = ?", [$data->Product_Name, $this->brunch])->row();
        echo json_encode($query);
    }
    public function check_variantwiseproduct()
    {
        $data = json_decode($this->input->raw_input_stream);
        $query = $this->db->query("SELECT p.* FROM tbl_product p WHERE p.status = 'a' AND p.Product_Name = ? AND p.ProductCategory_ID = ? AND p.brand = ? AND p.color = ? AND p.size = ? AND p.Product_branchid = ?", [$data->Product_Name, $data->category_id, $data->brand_id, $data->color_id, $data->size_id, $this->brunch])->row();
        echo json_encode($query);
    }
    ...
    public function updateProduct()
    {
        ...
        try {
            ...
            $productNameCount = $this->db->query("select * from tbl_product where Product_Name = ? and ProductCategory_ID = ? and brand = ? and color = ? and size = ? and Product_SlNo != ?", [$productObj->Product_Name, $productObj->ProductCategory_ID, $productObj->brand, $productObj->color, $productObj->size, $productObj->Product_SlNo])->num_rows();
            if ($productNameCount > 0) {
                $res = ['success' => false, 'message' => 'Product name already exists'];
                echo json_encode($res);
                exit;
            ...
            $productModelCount = $this->db->query("select * from tbl_product where model = ? and Product_SlNo != ?", [$productObj->model, $productObj->Product_SlNo])->num_rows();
            if ($productModelCount > 0) {
                $res = ['success' => false, 'message' => 'Product Model already exists'];
                echo json_encode($res);
                exit;
            }
    ...
    public function getProducts()
    {
        ...
        if (isset($data->model) && $data->model != '') {
            $clauses .= " and p.model = '$data->model'";
        }
        ...
        $products = $this->db->query("
            select
                p.*,
                concat(p.Product_Code, ' - ', p.Product_Name, ' - ', p.model, ' - ', c.color_name, ' - ', s.name) as display_text,
            ...
            left join tbl_size s on s.id = p.size
            where p.status = 'a'
            $clauses
    ...
    public function getTotalStock()
    {
        ...
        $stock = $this->db->query("
            select
                ...
                (
                    select 
                        ifnull(sum(dd.quantity), 0)
                    from tbl_damage_returndetails dd
                    where dd.status = 'a'
                    and dd.branch_id = '$branchId'
                    and dd.product_id = p.Product_SlNo
                ) as damage_return,
                ...
                (
                    select
                        ifnull(sum(e.quantity), 0)
                    from tbl_exchange e
                    join tbl_salesmaster sm on sm.SaleMaster_SlNo = e.sale_id
                    where e.status = 'a'
                    and e.product_id = p.Product_SlNo
                    " . (isset($data->date) && $data->date != null ? " and sm.SaleMaster_SaleDate <= '$data->date'" : "") . "
                ) as exchange_quantity,
                        
                (select (purchased_quantity + sales_returned_quantity + transferred_to_quantity + damage_return + exchange_quantity) - (sold_quantity + purchase_returned_quantity + damaged_quantity + transferred_from_quantity)) as current_quantity,
            ...
            join tbl_size s on s.id = p.size
...
<?php
    ...
    public function getProductLedger()
    {
        ...
        $result = $this->db->query("
            ...
            select 
                ...
                e.id,
                sm.SaleMaster_SaleDate as date,
                concat('Exchange - ', sm.exchange_note) as description,
                0 as rate,
                e.quantity as in_quantity,
                0 as out_quantity
            from tbl_exchange e
            join tbl_salesmaster sm on sm.SaleMaster_SlNo = e.sale_id
            where e.product_id = " . $data->productId . "
            and e.branch_id = " . $this->brunch . "
    ...
    public function autoBarcodeGenerate($productId)
    {
        $product = $this->db->query("
            ...
            join tbl_color c on c.color_SiNo = p.color
            join tbl_size s on s.id = p.size
            left join tbl_brand AS b ON b.brand_SiNo = p.brand
            where pd.Status = 'a'
            and pd.Product_IDNo = ?
        ", $productId)->row();
