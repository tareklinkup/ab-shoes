...
<div id="stock">
	...
	<div class="row" v-if="searchType != null" style="display:none" v-bind:style="{display: searchType == null ? 'none' : ''}">
		<div class="col-md-12">
			...
			<a href="javascript:" v-if="searchType == 'current'" onclick="exportToExcel('current_stock_table', 'Current Stock - <?php echo date('D d F, Y') ?>')">
				<i class="fa fa-file-excel-o"></i> Current Stock Excel
			</a>
			<a href="javascript:" v-else onclick="exportToExcel('total_stock_table', 'Total Stock - <?php echo date('D d F, Y') ?>')">
				...
				<table id="current_stock_table" class="table table-bordered" v-if="searchType == 'current'" style="display:none" v-bind:style="{display: searchType == 'current' ? '' : 'none'}">
					...
					<tbody>
						<tr v-for="product in stock">
							...
							<td>{{ product.sale_value | decimal }}</td>
					...
					<tfoot>
						<tr>
							...
							<th>{{ (stock.reduce((prev,curr) => {return +prev + +curr.Product_SellingPrice},0)).toFixed(2) }}</th>
							<th>{{ (stock.reduce((prev,curr) => {return +prev + +curr.Product_Purchase_Rate},0)).toFixed(2) }}</th>
							<th>{{ (stock.reduce((prev,curr) => {return +prev + +curr.sale_value},0)).toFixed(2) }}</th>
							<th>{{ totalStockValue | decimal }}</th>
						</tr>
					</tfoot>
				</table>

				<table class="table table-bordered" id="total_stock_table" v-if="searchType != 'current' && searchType != null" style="display:none;" v-bind:style="{display: searchType != 'current' && searchType != null ? '' : 'none'}">
					<thead>
						<tr>
							<th>Product Id</th>
							<th>Product Name</th>
					...
					<tfoot>
						<tr>
							<th colspan="17" style="text-align:right;">Total Stock Value</th>
							<th>{{ totalStockValue | decimal }}</th>
						</tr>
					</tfoot>
				</table>
