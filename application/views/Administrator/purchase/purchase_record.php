...
<div id="purchaseRecord">
	<div class="row" style="border-bottom: 1px solid #ccc;padding: 3px 0;">
		<div class="col-md-12">
			<form class="form-inline" id="searchForm" @submit.prevent="getSearchResult">
				...
				<div class="form-group" style="display:none;" v-bind:style="{display: searchType == 'brand' && brands.length > 0 ? '' : 'none'}">
					<label>Brand</label>
				...
				<div class="form-group" style="display:none;" v-bind:style="{display: searchType == 'memo' ? '' : 'none'}">
					<label>Memo</label>
					<v-select v-bind:options="memos" v-model="selectedMemo" label="memo_no"></v-select>
				</div>
	...
	<div class="row" style="margin-top:15px;display:none;" v-bind:style="{display: purchases.length > 0 ? '' : 'none'}">
		<div class="col-md-12" style="margin-bottom: 10px;">
			...
			<a href="javascript:" onclick="exportToExcel('purcahseRecord', 'Purchase Record - <?php echo date('D d F, Y') ?>')">
				<i class="fa fa-file-excel-o"></i> Purchase Record
			</a>
		</div>
		<div class="col-md-12">
			<div class="table-responsive" id="reportContent">
				<table id="purcahseRecord" class="record-table" v-if="(searchTypesForRecord.includes(searchType)) && recordType == 'with_details'" style="display:none" v-bind:style="{display: (searchTypesForRecord.includes(searchType)) && recordType == 'with_details' ? '' : 'none'}">
					<thead>
						<tr>
							<th>Invoice No.</th>
							<th>Date</th>
					...
					<tbody>
						<template v-for="purchase in purchases">
							<tr>
								...
								<td>{{ purchase.purchaseDetails[0].model }}</td>
								<td>{{ purchase.purchaseDetails[0].brand_name }}</td>
								<td>{{ purchase.purchaseDetails[0].color_name }}</td>
								...
								<td onClick="this.style.background = 'rgb(124 249 209)'" style="text-align:center;">
									<a title="Barcode" target="_blank" :href="`/Administrator/products/autoBarcodeGenerate/${purchase.purchaseDetails[0].Product_IDNo}`"><i class="fa fa-barcode"></i></a>
							...
							<tr v-for="(product, sl) in purchase.purchaseDetails.slice(1)">
								...
								<td onClick="this.style.background = 'rgb(124 249 209)'" style="text-align: center;">
									<a target="_blank" title="Barcode" :href="`/Administrator/products/autoBarcodeGenerate/${product.Product_IDNo}`"><i class="fa fa-barcode"></i></a>
								</td>
							</tr>
							<tr style="font-weight:bold;">
								<td colspan="9" style="font-weight:normal;"><strong>Note: </strong>{{ purchase.PurchaseMaster_Description }}</td>
								...
								<td style="text-align:right;">
									...
									Transport Cost: {{ purchase.PurchaseMaster_Freight }}<br>
									Labour Cost: {{ purchase.PurchaseMaster_labour_cost }}<br>
						...
						<tr style="text-align: center;background:#898989c2;color:white;">
							<td colspan="10" style="text-align: right;font-weight:700;">
								Total Amount:
							</td>
							<td style="text-align: right;">
								<span style='font-weight:700;color:white;'>{{ (purchases.reduce((prev, curr) => {return prev + parseFloat(curr.PurchaseMaster_TotalAmount)}, 0).toFixed(2)) }}</span>
							</td>
							<td></td>
						</tr>
					</tbody>
				...
				<table id="purcahseRecord" class="record-table" v-if="(searchTypesForRecord.includes(searchType)) && recordType == 'without_details'" style="display:none" v-bind:style="{display: (searchTypesForRecord.includes(searchType)) && recordType == 'without_details' ? '' : 'none'}">
					<thead>
						<tr>
							<th>Invoice No.</th>
							<th>Memo No.</th>
					...
					<tbody>
						<tr v-for="purchase in purchases">
							...
							<td style="text-align:right;">{{ purchase.PurchaseMaster_labour_cost }}</td>
					...
					<tfoot>
						<tr style="font-weight:bold;">
							...
							<td style="text-align:right;">{{ purchases.reduce((prev, curr)=>{return prev + parseFloat(curr.PurchaseMaster_labour_cost)}, 0) }}</td>
	...
	new Vue({
		...
		data() {
			return {
				...
				selectedMemo: null,
				purchases: [],
				searchTypesForRecord: ['', 'user', 'supplier', 'memo'],
		...
		methods: {
			...
			getPurchaseRecord() {
				let filter = {
					...
					memoId: this.selectedMemo == null ? '' : this.selectedMemo.memo_no,
