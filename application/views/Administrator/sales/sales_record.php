...
<div id="salesRecord">
	...
	<div class="row" style="margin-top:15px;display:none;" v-bind:style="{display: sales.length > 0 ? '' : 'none'}">
		<div class="col-md-12" style="margin-bottom: 10px;">
			...
			<a href="javascript:" onclick="exportToExcel('saleRecord', 'Sale Record - <?php echo date('D d F, Y') ?>')">
				<i class="fa fa-file-excel-o"></i> Sale Record
			</a>
		</div>
		<div class="col-md-12">
			<div class="table-responsive" id="reportContent">
				<table id="saleRecord" class="record-table" v-if="(searchTypesForRecord.includes(searchType)) && recordType == 'with_details'" style="display:none" v-bind:style="{display: (searchTypesForRecord.includes(searchType)) && recordType == 'with_details' ? '' : 'none'}">
					<thead>
						<tr>
							<th>Invoice No.</th>
							<th>Date</th>
					...
					<tbody>
						<template v-for="sale in sales">
							<tr>
								...
								<td>{{ sale.saleDetails[0].model }}</td>
								<td>{{ sale.saleDetails[0].color_name }}</td>
								<td>{{ sale.saleDetails[0].size_name }}</td>
						...
						<tr style="text-align: center;">
							...
							<td style="text-align: center;">
								<span style='font-weight:700;'>{{ (sales.reduce((prev, curr) => {return prev + +curr.saleDetails.reduce((p,c)=>{return +p + +c.SaleDetails_TotalQuantity},0)}, 0).toFixed(2)) }}</span>
							</td>
							<td style="text-align: right;">
								<span style='font-weight:700;'>{{ (sales.reduce((prev, curr) => {return prev + parseFloat(curr.SaleMaster_TotalSaleAmount)}, 0).toFixed(2)) }}</span>
							</td>
							<td></td>
						</tr>
					</tbody>
				...
				<table id="saleRecord" class="record-table" v-if="(searchTypesForRecord.includes(searchType)) && recordType == 'without_details'" style="display:none" v-bind:style="{display: (searchTypesForRecord.includes(searchType)) && recordType == 'without_details' ? '' : 'none'}">
					<thead>
						<tr>
							<th>Invoice No.</th>
							<th>Date</th>
						...
						<tbody>
							<template v-for="sale in sales">
								...
								<tr style="font-weight:bold">
									...
									<td style="text-align:right;">{{ sale.products.reduce((prev,curr) => {return +prev + +curr.quantity},0) }}</td>
								</tr>
							</template>
						</tbody>
					</table>
