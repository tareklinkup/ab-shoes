							...
							<form v-on:submit.prevent="addToCart">
								<div class="form-group">
									...
									<div class="col-xs-8">
										...
										<input v-else type="text" v-model="barcodeValue" @input="changeBarcode" placeholder="Barcode" ref="barcode" class="form-control" id="barcode">
									</div>
									<div class="col-xs-1" style="padding: 0;">
	...
	new Vue({
		...
		methods: {
			async changeBarcode() {
				...
				if (this.isProductFilterByBarcode && this.barcodeValue != '' && this.barcodeValue.length == 6) {
					let filteredProducts = this.products.filter(p => p.Product_Code == this.barcodeValue);

					if (filteredProducts.length) {
						this.selectedProduct = filteredProducts[0];
							...
							await axios.post('/get_product_stock', {
								...
								.then(res => {
									if (res.data > 0) {
										...
										this.selectedProduct.Product_SellingPrice = parseFloat(this.selectedProduct.Product_SellingPrice).toFixed(2);
										this.selectedProduct.total = parseFloat(this.selectedProduct.quantity * this.selectedProduct.Product_SellingPrice).toFixed(2)
