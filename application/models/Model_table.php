...
class Model_Table extends CI_Model
{
    ...
    public function generateSalesInvoice()
    {
        ...
        if ($sales->num_rows() != 0) {
            ...
            $invoice = date('ymd') .  (strlen($newSalesId) > count($zeros) ? $newSalesId : $zeros[count($zeros) - strlen($newSalesId)] . $newSalesId);
        }
    ...
    public function generateCustomerCode()
    {
        ...
        if ($lastCustomer->num_rows() != 0) {
            ...
            $zeros = array('0', '00', '000', '0000');
            $customerCode = 'AB' . (strlen($newCustomerId) > count($zeros) ? $newCustomerId : $zeros[count($zeros) - strlen($newCustomerId)] . $newCustomerId);
        }
    ...
    {
        $productModel = "AB00001";

        // $lastProduct = $this->db->query("select * from tbl_product order by Product_SlNo desc limit 1");

        $lastProduct = $this->db->query("SELECT `model` FROM `tbl_product` ORDER by model desc LIMIT 1")->row();
        if ($lastProduct) {
            $newProductId = substr($lastProduct->model, 2) + 1;

            $zeros = array('0', '00', '000', '0000');
            $productModel = 'AB' . (strlen($newProductId) > count($zeros) ? $newProductId : $zeros[count($zeros) - strlen($newProductId)] . $newProductId);
        }
    ...
    public function generateSupplierCode()
    {
        ...
        if ($lastSupplier->num_rows() != 0) {
            ...
            $zeros = array('0', '00', '000', '0000');
            $supplierCode = 'ABS' . (strlen($newSupplierId) > count($zeros) ? $newSupplierId : $zeros[count($zeros) - strlen($newSupplierId)] . $newSupplierId);
        }
    ...
    public function currentStock($clauses = '')
    {
        $stock = $this->db->query("
            select * from(
                select
                    ci.*,
                    (select (ci.purchase_quantity + ci.sales_return_quantity + ci.transfer_to_quantity + ci.damage_return + ci.exchange_quantity) - (ci.sales_quantity + ci.purchase_return_quantity + ci.damage_quantity + ci.transfer_from_quantity)) as current_quantity,
                    p.Product_Name,
                    p.Product_Code,
                    p.model,
                    c.color_name,
                    ...
                    (select (p.Product_SellingPrice * current_quantity)) as sale_value,
    ...
    public function supplierDue($clauses = "", $date = null)
    {
        ...
        $supplierDues = $this->db->query("
            ...
            (
                select 
                    ifnull(sum(dd.damage_amount) ,0)
                from tbl_damagedetails dd
                join tbl_damage dm on dm.Damage_SlNo = dd.Damage_SlNo
                where dd.status = 'a'
                and dd.supplier_id = s.Supplier_SlNo
                " . ($date == null ? "" : " and dm.Damage_Date < '$date'") . "
            ) as damage_amount,

            (
                select 
                    ifnull(sum(drd.amount), 0)
                from tbl_damage_returndetails drd
                join tbl_damage_return dr on dr.id = drd.return_id
                join tbl_damage d on d.Damage_InvoiceNo = dr.invoice
                join tbl_damagedetails dd on dd.Damage_SlNo = d.Damage_SlNo
                where drd.status = 'a'
                and dd.supplier_id = s.Supplier_SlNo
            ...
            (select (bill + cashReceived + damage_amount) - (paid + returned + damage_return)) as due
    ...
    public function productStock($productId)
    {
        ...
        if ($stockCount != 0) {
            ...
            $stock = ($stockRow->purchase_quantity + $stockRow->transfer_to_quantity + $stockRow->sales_return_quantity + $stockRow->exchange_quantity + $stockRow->damage_return)
